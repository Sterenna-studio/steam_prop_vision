...
def download(model_name: str = DEFAULT_MODEL) -> Path:
    ...
    if dest.exists():
        print(f"[model] {dest.name} déjà présent ({dest.stat().st_size // 1024} Ko) — skip.")
        return dest
    ...
    try:
        ...
        if candidates:
            ...
            if cwd_file.exists():
                ...
                print(f"[model] ⚠️  Fichier non trouvé après téléchargement. Vérifier manuellement.")
                sys.exit(1)
    except ImportError:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default=DEFAULT_MODEL, help="Nom du modèle (ex: yolov8n.pt)")
