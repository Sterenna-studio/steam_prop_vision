...
def download(model: str = DEFAULT_MODEL, dest: Path = DEFAULT_DEST) -> Path:
    ...
    if target.exists():
        ...
        print(
            f"[download_model] ❌ Échec du téléchargement de {model}", file=sys.stderr
        )
        sys.exit(1)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Télécharge le modèle YOLO pour S.T.E.A.M Vision"
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        "--dest", default=str(DEFAULT_DEST), help="Dossier destination (défaut: .)"
    )
